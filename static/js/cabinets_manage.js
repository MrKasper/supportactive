    ...
    function loadCabinetsManagePage() {
        ...
        fetch('/api/directory/cabinets')
            ...
            .then(function(cabinets) {
                if (cabinets.error) {
                    $contentBlock.html('<div class="alert alert-danger">' + utils.escapeHtml(cabinets.error) + '</div>');
                    return;
                }
                ...
                html += '<button class="btn btn-sm btn-light" onclick="loadCabinetsManagePage()"><i class="bi bi-arrow-clockwise"></i> Обновить</button>';
                html += '<button class="btn btn-sm btn-success" onclick="showAddCabinetModal()"><i class="bi bi-plus-circle"></i> Добавить кабинет</button>';
                html += '</div></div></div>';
