...
(function() {
    ...
    function showCartridgeMonthDetails(monthKey) {
        ...
        if (replacements.length === 0) {
            Swal.fire({ icon: 'info', title: monthName + ' ' + year, text: 'В этом месяце замен не было' });
            return;
        }
        ...
        var html =
            ...
            '<div class="col-6"><div class="p-2 rounded" style="background:rgba(102,126,234,.1)">' +
            '<div style="font-size:1.4rem;font-weight:800;color:#667eea">' + replacements.length + '</div>' +
            '<div style="font-size:.7rem;color:#888;text-transform:uppercase">Всего замен</div>' +
            '</div></div>' +
            '<div class="col-6"><div class="p-2 rounded" style="background:rgba(40,167,69,.1)">' +
            '<div style="font-size:1.4rem;font-weight:800;color:#28a745">' + Object.keys(byCabinet).length + '</div>' +
            '<div style="font-size:.7rem;color:#888;text-transform:uppercase">Кабинетов</div>' +
            '</div></div></div>' +
            ...
            '<thead><tr><th>Дата</th><th>Кабинет</th><th>Принтер</th><th>Картридж</th></tr></thead>' +
    ...
    function renderTable(cartridges) {
        ...
        if (cartridges.length === 0) {
            html += '<tr><td colspan="7" class="text-center text-muted py-4">Нет записей</td></tr>';
        } else {
    ...
    console.log('[cartridges] Загружено (без ID)');
})();