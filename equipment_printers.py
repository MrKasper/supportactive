...
# ============================================================
# ⚠️ В модуль «Картриджи» (таблица cartridges) пишется только ДАТА
#    формата 'YYYY-MM-DD'. Поле replacement_dates — CSV-строка.

def _append_replacement_date(dates_str, new_date):
    """
    Добавляет дату к CSV-строке replacement_dates.
    new_date должен быть в формате 'YYYY-MM-DD'.
    """
...
def _sync_cartridge_replacement(tx, cabinet_number, printer_model,
                                 new_cartridge, responsible, replacement_date):
    """
    Синхронизирует замену картриджа с таблицей `cartridges`
    (модуль «Картриджи»).

    Параметр replacement_date — СТРОКА вида 'YYYY-MM-DD' (без времени).
    ...
    if existing:
        new_dates = _append_replacement_date(
            existing['replacement_dates'], replacement_date
        )
        tx.execute(
...
def replace_cartridge(printer_id):
    try:
        ...
        log.info(
            ...
            f'{old_cartridge} → {cartridge} '
            f'(date={today}, sync={sync_info.get("action")})'
        )
