...
API доступен Администратору и Разработчику.
Страница /audit — только для Разработчика.
"""
...
# Роли, которым разрешён доступ к API журнала
AUDIT_ROLES = (ROLE_ADMIN, ROLE_DEVELOPER)

# Роли, которым доступна страница /audit
AUDIT_PAGE_ROLES = (ROLE_DEVELOPER,)
    ...
    try:
        ...
        db.execute('''
            INSERT INTO task_history
                (task_id, user_id, user_name, field_name,
...
@role_required(*AUDIT_PAGE_ROLES)
def audit_page():
...
def get_audit_stats():
    try:
        total = db.query(
            'SELECT COUNT(*) AS count FROM audit_log', one=True
        )['count']
        ...
        ''')
        top_users = db.query('''
            SELECT user_login, COUNT(*) AS count FROM audit_log
            WHERE user_login != ''
            GROUP BY user_login ORDER BY count DESC LIMIT 10
        ''')
